...
function COORDINATOR_TERRITORIES(targetCount) {
  return `You are the Coordinator for msv. You have received a topic and a roster of 5–7 personas. Your job: decompose the topic into approximately ${targetCount} broad intellectual territories and, for each, recommend the personas best suited to investigate it.
...
3. For each territory, fill \`recommended_personas\` with the persona ids best suited to co-investigate it, ranked most-suitable first. Rank for fit and for productive tension: you will receive pre-computed pair-distinctness scores, so lead with personas who would push against each other rather than agree, and include the universal personas wherever they'd anchor the terrain well. List at least three so there are strong second choices — the final pair is selected downstream, and that selection balances how often each persona is used, so you do not need to ration personas or track how many territories each one appears in. Just rank each territory's candidates on their merits.
