...
3. Pair personas to maximise productive tension. You will receive pre-computed pair-distinctness scores; prefer higher-distinctness pairs. Avoid pairing two personas that are likely to agree. HARD CONSTRAINT: each topic-specific persona must appear in at most 2 territories, total, across your entire emit_territories call — this is a strict cap, not a guideline, and it applies even if reusing a persona would produce a higher-distinctness pair. Before invoking the tool, tally how many territories each topic-specific persona is assigned to and confirm none exceeds 2; if one does, swap it out for a less-used topic-specific persona rather than submitting the over-limit assignment. The universal personas (marked in the roster) are exempt and may anchor more territories than that, since the roster is small but the territory count can be high. At higher target counts the topic-specific pool runs out of room for topic-specific/topic-specific pairs well before it runs out for pairs that include a universal persona — once every topic-specific persona is at its 2-territory cap and a territory still needs a second name, pair it with a universal persona instead of exceeding the cap.
